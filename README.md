...
3. Builds an affiliate link using your deep-link prefix, strips any original URLs, and guarantees your personal link appears once in every post (with an extra safety append if the model omits it).
...
- Every post uses the AliExpress URL from the source message, but it is always wrapped by your **`AFFILIATE_PREFIX`** secret (deep-link prefix from your partner network). That produces a personal, tracked link for each product.
...
Guardrails: Hebrew only, 1–3 emojis total, slightly livelier tone with light humor, no made-up data, and skips sections when details are missing. The model is told to avoid extra links, and the bot strips non-affiliate URLs if they sneak in.
