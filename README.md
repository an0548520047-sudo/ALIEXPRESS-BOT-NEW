...
- **מצב מקוצר וחובה פורטל** – אפשר לאכוף שימוש רק ב-API/תבנית מהפורטל (ללא Prefix) ואפילו לדרוש שלינק מקוצר יחזור בהצלחה, אחרת הריצה נופלת מוקדם כדי שלא יתפרסמו לינקים לא אישיים.
...
- **סידור טקסט חכם** – מנקה שורות כפולות ורווחים מיותרים כדי שהפוסט הסופי יהיה קצר, אחיד וללא בלגן סביב בלוק הלינק.
...
4. מפיק לינק שותף אישי מה־API/תבנית/Prefix, ומסיר כל קישור זר. אם ה־API
   ...
   אם לא נמצא מזהה מוצר תקין (ספרות, אורך ≥6) – הפוסט מדולג כדי למנוע הפניות
   לעמודי 404 או כפילויות לא רלוונטיות.
...
- אם הקריאה ל־API של הפורטל גורמת לעיכוב: אפשר להוריד את `AFFILIATE_API_TIMEOUT` (ברירת מחדל 5 שניות, חייב להיות חיובי) או להשאיר את `AFFILIATE_API_ENDPOINT` ריק כדי לעבור אוטומטית לתבנית/Prefix. אם `REQUIRE_PORTAL_AFFILIATE=true`, חובה שאחד מהם יחזיר לינק תקין אחרת הריצה תעצור.
...
- אם ה-Prefix שלך כבר מוסיף קידוד או יוצר לינק קצר, הגדר `AFFILIATE_PREFIX_ENCODE=false` כדי למנוע קישורים ארוכים מקודדים פעמיים.
- אם אתה חייב לינקים קצרים בלבד – הגדר `REQUIRE_SHORT_LINKS=true` יחד עם `AFFILIATE_SHORTENER_ENDPOINT`; אם הקיצור נכשל או לא שייך למוצר, הריצה תפסיק כדי שלא יתפרסם לינק ארוך.
