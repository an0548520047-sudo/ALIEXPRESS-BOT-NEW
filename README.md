...
| `AFFILIATE_API_TIMEOUT` | לא | ברירת מחדל 5 שניות (זמן מקס' לחיבור/קריאה מה־API) |
...
> ערכים מספריים אופציונליים: אם תשאיר ריק/רווחים בלבד, ייקח ברירת מחדל; אם תזין טקסט לא מספרי – תודפס אזהרה והוא יחזור לברירת המחדל.
...
- אם הקריאה ל־API של הפורטל גורמת לעיכוב: אפשר להוריד את `AFFILIATE_API_TIMEOUT` (ברירת מחדל 5 שניות) או להשאיר את `AFFILIATE_API_ENDPOINT` ריק כדי לעבור אוטומטית לתבנית/Prefix.
- אם אחד המספרים מוגדר עם טקסט שגוי, הריצה לא תיעצר – תודפס אזהרה והערך יחזור לברירת המחדל.
